> ⚠️ Status: DRAFT — never executed end-to-end. Created 2026-04-22, parked until 
> validated. Do not use as if this were a live workflow. See vault CLAUDE.md for 
> the current Monday process.

---
name: sostocked-full-automation
description: >
  Full end-to-end SoStocked automation. Downloads all 6 required files
  (3 Projected Forecast Models + 3 Inventory Current Views) across all three
  brands (SS, MTB, NFMD), runs combine_forecast.py to build the Weekly_Forecast
  workbook, then runs demand_planning.py to produce the demand plan.
...
After Phase 2, `~/Downloads` should contain 6 new files:
