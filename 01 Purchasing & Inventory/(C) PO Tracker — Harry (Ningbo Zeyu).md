---
type: po-tracker
vendor: Ningbo Zeyu Industry & Trade (Harry Zhan · harryzhan@nb-zeyu.com)
source: Outlook sent + received (tom@michaeltoddbeauty.com)
last_scanned: 2026-08-06
status: living
---

# 📑 PO Tracker — Harry / Ningbo Zeyu

Living tracker of POs, built from sent + received email. **Stage** follows the
[[06 Processes & SOPs/(C) PO Approval Workflow SOP]]:
`① SAP entered · ② PO→Harry · ③ PI←Harry · ④ adjusting · ⑤ validated (PPO Validator) · ⑥ with approvers (Lilia/Donna/SC) · ⑦ approved→Michael · ⑧ signed→Harry = LIVE`

| PPO # | Brand | Item / notes | Stage | Next action |
|---|---|---|---|---|
| **3263** | MTB | US / UNIS / CA (`PPO3263-MTB-US-UNIS-CA.pdf`) | **⑧ LIVE** — signed copy sent to Harry 7/27 (PI first mislabeled **3221** → corrected) | Confirm production/ship schedule |
| **3264** | SPA | US / UNIS / CA (`PPO3264-SPA-US-UNIS-CA.pdf`) | **⑧ LIVE** — signed copy sent to Harry 7/27 | Confirm production/ship schedule |
| **3267** | NFMD | US / UNIS / CA (`PPO3267-NFMD-US-UNIS-CA.pdf`) | **⑧ LIVE** — signed copy sent to Harry 7/27 (revised PI 3267) | Confirm production/ship schedule |
| **3273** | MTB | Floship — SonicSmooth handles + replacement (barcodes sent 7/27) | **⑧ RECEIVED at Floship** — Harry **8/3**: signed for; 1 big carton = 4 small (25 handles each = **100 handles**) | Done — landed at Floship |
| **3276** | MTB | **SonicSmooth Blades `811573031335`** — 30,000u (the SVP's 30k blade PO) | **⑧ SIGNED 7/30** — Michael signed `PPO3276-MTB-SSBLADES.pdf`, Harry CC'd on the approval | ✅ Confirm the signed copy is formally to Harry = LIVE. Covers ShipBob's **December**. |
| **3206** | MTB | **CA SonicSmooth Blades — 1,500u** (ASG-MTB / Alliance CA staging) | in transit — Harry **7/30** confirmed "CA blade only 1500pcs"; CA shipment proceeding **$7,935 DDU** to Alliance Brampton (Augusto 7/30) | Track landing at Alliance (feeds Amazon CA) |

## Other vendors (non-Harry)
| PPO # | Vendor | Item | Stage | Next action |
|---|---|---|---|---|
| **3236** | YAC Chemicals (Gloria) | Hair Identifier Spray `YC8441` | ③/④ PI + **artwork revision** — Gloria **7/31** sent revised design; open Q: can artwork English-only vs color box 3-languages | Juan owns artwork; confirm language spec so YAC can finalize |
| — | Oxygen (Severino) | Glow Facial Oil | **formula APPROVED** 7/30; **artwork/label LOCKED 8/5** (master-shipper label centered over old barcode; batch code in flap + color-box bottom) | **Confirm pickup 8/7** holds |
| **3167** | Emily / BMC (Zhiwen) | (shipment issue thread) | received & counting at warehouse (Emily 7/31); Augusto out next wk → forwarder to route via supplychain@ | Confirm final count + delivery schedule |

## Open threads / notes
- **🔴 BLADE FLY-IN — Harry's schedule set, needs per-DC split (Harry 8/5):** **25,050 blades fly Aug 15** · **25,050 fly Aug 29** · **5k–10k by sea Aug 22 & 29**. **Sima blade** = Tom yes/no on flying (fly → ready 8/25; else stays 8/29 sea). Harry needs **qty per warehouse + each warehouse address** to quote air cost. Donna's split = **50% Fairburn GA · 27% PA · 23% Reno**. ⚠ ShipBob rule: **cannot air directly to spoke sites** — must inbound via a receiving hub (West/Moreno Valley or NE) then IPP to spokes (one WRO per region, auto-distribute OFF), with hub→spoke SLA lag. → **Action Tom: send Harry the per-DC qty + addresses.**
- **🚢 Container ONEU6390520 (the "4th Aug" / CI 039 — SonicSmooth blade container) — SHIPPED to UNIS (Harry 8/5).** DO issued (Harry + Nancy@siyang-china); **seal # CNEB38651**; delivery = **UNIS (new warehouse — no truck)**. Donna running drayage (UNIS now wants `cadrayage@unisco.com` + `csr.drayage@item.com` cc'd on all future DOs). **3 cleanups (8/5):** (1) UNIS says container is **NOT marked as a transload in WISE** (RN-46515) — flag if UNIS→ShipBob transload intended; (2) Zach needs **correct Lot #s + CS qty for 850038082659**; (3) Harry revised CI = **3,186 loading / 1,890 left**, the **300-pc Soniclear Base (Michael urgent)** was on wrong UPC → corrected; **Lil flagged PO 3210 plan-vs-invoice mismatch (1,890 pcs)** — confirm w/ Harry.
- **🔴 BOAT DELAY (Harry 8/3):** the **Saturday container slipped to Tuesday-AM loading** (boat delayed) — ✅ now shipped as ONEU6390520 above. Harry updating remaining blade-container dates after they load → **ETAs shift ~3 days**; arrival maps move once he updates. Harry also asked **OK to ship PART first** — still needs Tom's yes/no.
- **✅ Container plan cols R/S wired for Harry (Tom 8/1):** col **R = qty sending** · col **S = confirmed date** · col **Q = notes** — Harry acknowledged 8/3. **Chase him to fill R/S** on the blade POs; the moment he does, every report firms up automatically. Still empty as of 8/3 AM.
- **Blade shortage is the priority** (SVP 7/29 + Lil's all-day review). **PO 3276 (30k) is now SIGNED/LIVE** — covers December. Near-term expedite: Michael **7/30** pushed Harry to add to a **~3,000-pc load on 8/1** ("we are VERY short"); Harry: "we will try… if we make more, we will load more." Harry's confirmed near-term = **3,000 Aug 8 + 5,250 Aug 15** (8,250). The Oct-4 containers **3221 + 3244** (25,050 each) load **8/20** — must sail on time. **SS blade transfers DN-2051712 / DN-2051703 are being picked at Unis (7/31, Michelle Topete)** — confirm they ship.
- **🔴 Batch-code sign-off (7/31) — blocks final PIs.** Harry updated all codes on the final PI: **3154 → new batch code (done, PI attached)**; **3152 still on old `0426` — Harry asks "do you need change?"**; **salt = `RGS0726`** (Harry pushed back on a floated `0826`); **device stays `0426`**. → Decide w/ Juan; details in [[01 Purchasing & Inventory/(C) Batch Code Tracker.md]].
- **Container Plan updated (Augusto 7/31):** Ready Date col L = PO+45 (estimate); **Harry's ACTUAL ready dates in his notes, col Q.** Real load/sail dates still need booking.
- **⚠ Augusto OOO all next week** — BMC/Emily 3167 delivery + all forwarder/quote traffic routes to **supplychain@** while he's out.
- **Salt (NasalFresh) batch/expiry codes:** new prefix codes locked with Harry — Regular = **RGS**, Xylitol = **XYS** (per Augusto). Juan updating artwork.
- PI quantities/prices for 3263/3264/3267/3276 live in the attached PDFs — say the word and I'll open them to fill exact qtys/$ into the table.

---
*Auto-built from Outlook on 2026-07-30 (last ~120 days, recipient = harryzhan@nb-zeyu.com). Re-run to refresh; each scan updates statuses as PIs/confirmations arrive.*
