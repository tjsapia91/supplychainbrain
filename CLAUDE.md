...
- ✅ **Monthly Flow (Amazon): min/max BATCH send-in from physical UNIS; POs NOT counted (Tommy 2026-08-06).** `Send-in → FBA` is a professional min/max policy: fire a batch **only when projected FBA drops below the reorder point**, then refill up to order-up-to — two editable band knobs **`Reorder ≤ (d)`** (default 30) + **`Up-to (d)`** (default 90) → **lumpy** sends (months of 0), ~60d avg. Draws from **`UNIS on-hand (covers)`** = prior on-hand **+ POs landed this month** − send-in. **🔒 SETTLED: a PO is informational while in transit but BECOMES UNIS stock on its arrival month** (Tommy: "the po becomes unis stock once it lands") — so landed POs feed the pool the send-in draws on; nothing counts before it lands or goes straight to FBA. Dropped the confusing "UNIS potential (+POs)" running row. Verified on Lavender: send-in fires 20,505/24,477 as POs land, stockout Oct-30-2026 → Mar-24-2027. COM-verified 0 errors; cadence 15,904/0/19,403/0/8,373/0… `DEFAULT_TARGET_DOC`/`DEFAULT_TRIGGER_DOC` knobs. ShipBob flow unchanged (PO covers directly). **Ending (FBA) = Starting + Send-in + Fly-in − Forecast.** Raising the target = bigger send-in / faster UNIS drawdown. COM-verified 0 errors; Hair Spray send-in scales 1,388→37,678 as target 30d→180d; send-in/pool formulas reference physical UNIS only. `DEFAULT_TARGET_DOC` knob. ShipBob flow unchanged (PO covers directly).
- ✅ **NEW Decision Board (`build_decision_board.py`) — demand planning + the 3 decisions, TWO tabs (Amazon / ShipBob) (Tommy 2026-08-06).** ONE row per SKU, **worst-first**, per-tab action-count banner. **THREE tabs: Amazon · ShipBob · Walmart** (Amazon = UNIS→FBA; ShipBob = Walmart/Shopify/TikTok/Floship; **Walmart = fulfilled FROM ShipBob** — its rows are the ShipBob rows filtered to Walmart SKUs (SS/NFMD; MTB has none) with Walmart's own /mo pull shown next to the shared ShipBob position/stockout). Send-in sourced **UNIS-first then ShipBob** (shows the split); **exact stockout DAY** (day-level interpolation) off **On hand=FBA+AWD + UNIS + ShipBob + landed POs**. Columns: Node · Product · Class · Status · Demand/mo · On hand · On order · Days cover · Stockout · **Action** (plain English: "Send in from UNIS" / "Order N" / "⚠ Expedite/fly + order N" / "OK"). Reuses the Monthly Flow's exact helpers (`F._rows_for/_incoming/_po_by_month/_status_tag`) so numbers never drift; order qty = order-up-to (A 300d/B 240/C·D 180) − total position. Excel AutoFilter for slice-by-node/status. First run: **220 SKUs, 49 need action**. Published to the SUPPLY CHAIN ANALYSIS hub. **Send-in columns:** "Send-in" = units to move now + "Cover after" = days it buys; **sourced UNIS-first then ShipBob free-to-transfer** (Tommy: use ShipBob until UNIS holds the units) — action shows the split e.g. "Send in 5,917 (UNIS 0 + ShipBob 5,917) → 90d". Send-in gap is off **FBA+AWD** (nets AWD). ShipBob-free counts in the reserve so bridgeable SKUs read 🟡 not 🟠. `SENDIN_TARGET_DAYS=90` knob. **4th tab = Transloads** (visibility): in-transit POs flagged in the in-transit-log **col M "Transload to UNIS/AWD"** (new `AP.load_transload_to_amazon()`, spelling-tolerant, +`TRANSLOAD_LEAD_DAYS=14`) — land at ShipBob then transload to Amazon (AWD-labeled→FBA/AWD, else UNIS reserve), available ≈ arrival+14d. Display only, **counted once** (Tommy: AWD inbound reconciles with transload notes + isn't double-counted → no routing change; reroute wiring built but left dormant). 6 rows: AWD 12,852u (incl. Lavender 10,368 = AWD-report Inbound) · UNIS 6,321u. ⚠ ShipBob demand relies on the ShipBob replen forecast aggregating all its channels — TikTok undercounts ~3-4× in Valogix, so ShipBob may read healthier than reality (verify). Trigger: *"build the decision board"*.
- ✅ **"Inbound to AWD" now captured + `NO_AWD_UPCS` mechanism (Tommy 2026-08-06).** 811573031090 (SonicSmooth Lavender) **IS** real AWD stock (10,044) — it's listed under **MTBlavendar** — so `NO_AWD_UPCS` is now **empty** (mechanism kept for genuinely-no-AWD SKUs). New `build_amazon_us_replen.load_awd_inbound()` reads the Seller Central **AWD Inventory Report** `Inbound to AWD (units)` per UPC (6 SKUs; Lavender 10,368); the flow shows it as an **"Inbound → AWD"** line in the incoming box. ⚠ **Not counted** into coverage — for Lavender the 10,368 inbound-to-AWD = the two In-Transit POs (4,536+5,832) exactly, i.e. the SAME units already counted (Amazon's view of where they land), NOT additional supply.
- ✅ **Monthly Flow (Amazon): FBA+AWD kept COMBINED + per-SKU status tags (Tommy 2026-08-06).** AWD auto-replenishes FBA so they're one Amazon pool ("keep it simple") — one `Starting FBA+AWD` row; UNIS min/max send-in tops it up (effectively kicks in as AWD runs down). (Briefly built a separate AWD→FBA cascade, then reverted per Tommy — combined is the intended model.) **Status tag per SKU** on the band from total position (FBA+AWD + UNIS + landed POs) vs `LEAD_DAYS`(140): 🟢 HEALTHY · 🟡 SEND-IN NEEDED · 🟠 ORDER SOON · 🔴 CRITICAL (this run 🟢17/🟡13/🟠11/🔴7). COM-verified 0 errors.
- ✅ **Monthly Flow visual cleanup (logic unchanged, Tommy 2026-08-06):** whole-number formatting (`#,##0`, kills decimal noise like 8,646.129); incoming box **collapsed to one line per (PO, type)** with summed qty + earliest dates (PO 3143 ×2 / PO 3233 ×3 → one each); **Ending + Days-of-cover bold** as the headline, PO→UNIS / UNIS on-hand muted. COM-verified 0 errors.
- ✅ **Hair Spray leads the SonicSmooth block** (`sku_rules.FAMILY_LEAD`/`family_lead_rank`) — within a family, lead keywords sort first (SonicSmooth: `HAIR`), rest by demand. Applied to flow + all 3 planners.
...
- *"Build the decision board"* — one-tab demand-planning + decision view (Amazon UNIS→FBA · ShipBob WMT/Shopify/TikTok · Order), worst-first with a plain-English action per SKU (`scripts/build_decision_board.py`). Published to the SUPPLY CHAIN ANALYSIS hub.
