...
- ✅ **`NO_AWD_UPCS` override — 811573031090 (SonicSmooth Lavender) not stocked at AWD (Tommy 2026-08-06).** The Amazon AWD Ledger carries a stale/phantom **10,044** balance for it (zero departures/33d) that inflated on-hand. New `NO_AWD_UPCS` set in `build_amazon_us_replen.load_us_onhand()` forces AWD=0 → on-hand 17,830→**7,786** (FBA only), correcting the replen planner + flow. ⚠ The AWD Ledger source still shows the phantom — worth cleaning at the source; add more UPCs to the set as needed.
