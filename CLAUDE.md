...
- ✅ **Monthly Flow (Amazon) remodeled to reality: PO → UNIS → send-in → FBA (Tommy 2026-08-06).** A landed PO **no longer credits FBA directly** — it lands at UNIS (row `PO → UNIS`) and builds the pool. New `Send-in → FBA (auto)` row **auto-sizes to a per-SKU editable `Target days cover` knob** (blue, on the band, default **60d**), capped by the pool (`UNIS pool left` tracks the drawdown). **Ending (FBA) = Starting + Send-in + Fly-in − Forecast**, so FBA stocks out only when UNIS (fed by POs) can't cover. Raising the target = bigger send-in / faster UNIS drawdown; lowering = smaller. COM-verified 0 errors; Hair Spray send-in scales 1,388→37,678 as target 30d→180d. `DEFAULT_TARGET_DOC` config knob. (Replaced the earlier one-shot "Your send-in" box input.) ShipBob flow unchanged (PO covers directly).
