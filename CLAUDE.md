...
- ✅ **Monthly Flow (Amazon) remodeled: physical UNIS → send-in → FBA; POs NOT counted (Tommy 2026-08-06).** New `Send-in → FBA (auto)` row **auto-sizes to a per-SKU editable `Target days cover` knob** (blue, on the band, default **60d**) and draws **ONLY from physical UNIS on-hand** (`UNIS on-hand (covers)` = prior − send-in). **`PO → UNIS (arriving)` (grey) shows POs arriving at UNIS = potential inventory**, and a **`UNIS potential (+POs)`** row (green italic, display) = physical + arriving POs − send-ins shows the upside — but POs do NOT feed FBA coverage (Tommy: "we don't want POs counting into coverage" but "show the pos going to unis as potential inventory"). **Ending (FBA) = Starting + Send-in + Fly-in − Forecast.** Raising the target = bigger send-in / faster UNIS drawdown. COM-verified 0 errors; Hair Spray send-in scales 1,388→37,678 as target 30d→180d; send-in/pool formulas reference physical UNIS only. `DEFAULT_TARGET_DOC` knob. ShipBob flow unchanged (PO covers directly).
