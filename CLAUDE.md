...
- ✅ **Monthly Flow (Amazon): FBA ← AWD ← UNIS cascade + per-SKU status tags (Tommy 2026-08-06).** Demand hits FBA; **AWD auto-replenishes FBA first** (`AWD→FBA = MIN(AWD, top-up need)`); **UNIS send-in covers only what AWD can't** (`MAX(0, need−AWD→FBA)`). FBA vs AWD split from `U.load_us_onhand()`; rows: FBA start · AWD→FBA · UNIS→FBA · Ending · Days · AWD on-hand · UNIS on-hand · PO→UNIS. **Status tag per SKU** on the band from total position vs `LEAD_DAYS`(140): 🟢 HEALTHY · 🟡 SEND-IN NEEDED · 🟠 ORDER SOON · 🔴 CRITICAL. COM-verified 0 errors; Lavender AWD drains 10,044 first, then UNIS 20,505/24,477 as POs land; tag counts 🟢17/🟡13/🟠11/🔴7.
