...
| Incoming (PO → CA) | supplier Open POs to `ASG-*`. Arrival at Alliance, **most-concrete-wins:** (1) **In-Transit Log** `WHSE DELV. DATE` once the PO has shipped — the source of truth; (2) container-plan **Load + 45d** (booked); (3) Open PO **Ship-By Date + 45d** (estimate); (4) Original Due Date. Then **sellable on Amazon CA = Alliance arrival + 25d** transfer. The workflow: follow the Open PO estimate → Container Plan once booked → In-Transit once it actually sails. | In-Transit Log + Container Plan + SAP Open POs |
